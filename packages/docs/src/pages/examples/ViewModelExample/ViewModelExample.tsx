...
export const ViewModelExample = () => (
  <PageBlock title="ViewModel">
    <ExampleBlock title={<>Typing <Code>parent</Code> and <Code>viewProps</Code></>} example={typingExample}>
      View models have link to their parents and also have link to view&apos;s props. And you can type both of these
      fields.
    </ExampleBlock>

    <ExampleBlock title={<>Using <Code>parent</Code></>} example={parentExample}>
      If a view is located somewhere inside another view, inner view can use outer one&apos;s view model.
    </ExampleBlock>

    <ExampleBlock title={<>Observing <Code>viewProps</Code></>} example={optionsExample}>
      The <Code>viewProps</Code> field updates every time the view is rendered with new props. The view is memoized, so
      the amount of updates is minimized. But you need to keep in mind a few rules when you want to
      observe <Code>viewProps</Code>. If you are using <Code>viewProps</Code> inside an <Code>autorun</Code> or an
      observer component (<Code>observer</Code>, <Code>view</Code> or <Code>childView</Code>), when the reaction will be
      called every time the view is rendered with new props.
    </ExampleBlock>

    <ExampleBlock title="View lifecycle hooks" example={hooksExample}>
    ...
    <ExampleBlock title="Creating reactions" example={reactionsExample}>
